...
def test_force_direction_matches_gymnasium_v1():
    """Lock the action↔force convention to the Gymnasium CartPole-v1 reference:
    action=1 pushes the cart in the +x direction, action=0 pushes in -x.
    Regression guard against an off-by-one sign flip (MiMo flagged this in
    a panel review; verifying here so the convention can't drift silently)."""
    from wmagent.world.cartpole import CartPoleParams, _cartpole_step

    p = CartPoleParams()
    rest = torch.tensor([0.0, 0.0, 0.0, 0.0]).numpy()
    next_right = _cartpole_step(rest, action=1, p=p)
    next_left = _cartpole_step(rest, action=0, p=p)
    # x_dot is index 1; from rest, action=1 should produce positive x_dot.
    assert next_right[1] > 0.0
    assert next_left[1] < 0.0
    # And they should be exactly antisymmetric around the rest state.
    assert abs(next_right[1] + next_left[1]) < 1e-6
