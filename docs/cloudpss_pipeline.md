...
This document records the public-safe CloudPSS practice used by
gridwm-agent. It is written for reproducibility. Do not place CloudPSS
passwords, tokens, browser cookies, private model links, or account
material in this repository.

## Role In gridwm-agent

CloudPSS is the EMT simulation oracle. gridwm-agent uses it to create trajectories
from an IEEE 39-bus system, then trains a neural latent recurrent dynamics model to
approximate short-horizon future dynamics under fault actions.

The resulting story is:

```text
CloudPSS EMT simulator -> HDF5 trajectory dataset -> latent recurrent dynamics model
...
gridwm-agent implements this chain in:
...
Default gridwm-agent collection settings are in `configs/data.yaml`:
...
aggressive parallelism. gridwm-agent therefore keeps collection reliable first and
...
that are useful for gridwm-agent, but they should remain extensions instead of
...
- **Energy-method validation**: a classical dissipated-energy heuristic can
  ...
  For gridwm-agent V2.1, this can become a downstream sanity check: if imagined
...
- **Noise and missing-data augmentation**: older dataset builders injected row
  noise and missing samples before training classifiers. For gridwm-agent, the better
...
CloudPSS is used as a high-fidelity EMT oracle that produces counterfactual
fault trajectories. The trained model approximates short-horizon EMT
dynamics on those trajectories — it is not a real-time deployment artefact
and has no protection-system certification.
