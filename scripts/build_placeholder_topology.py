"""Build the topology file the V2 model is trained against:
``data/graph_ieee39.h5`` with edge_index, edge_attr, node_attr.

WARNING — this is a *placeholder* topology. ``edge_attr`` and
``node_attr`` are zero-filled and the graph is fully connected. The
trained model therefore does NOT receive any IEEE-39 electrical prior
through edge or node features; the only graph signal it sees is
generator-pair adjacency, and that adjacency is uniform. All headline
numbers in this repo were trained against this placeholder graph. See
README → Caveats.
"""
