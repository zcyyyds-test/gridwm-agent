"""Create a README-ready table from gridwm-agent agent eval files.
...
def _ft7_table(rows: list[tuple[str, dict]]) -> list[str]:
    ...
    lines = [
        ...
        "The physics heuristic column is an unbiased rule-based strawman: "
        "rank candidates by `fault_type + duration/180 + 0.05/resistance` "
        "with no fault-type bonus.",
        "",
        "| Run | Oracle low | Agent safe | Random | Physics heuristic | "
        "Agent discover | Oracle high | Discover gain over heuristic |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for name, report in rows:
        # Accept old-format JSONs whose field is named ft7_heuristic_mean_risk.
        heuristic = report.get(
            "physics_heuristic_mean_risk",
            report.get("ft7_heuristic_mean_risk"),
        )
        oracle_high = report.get("oracle_high_mean_risk")
        agent_discover = report.get("agent_discover_mean_risk")
        if heuristic is None or oracle_high is None or agent_discover is None:
            gain = "n/a"
        else:
            denom = oracle_high - heuristic
            gain = f"{(agent_discover - heuristic) / denom * 100:+.1f}%" if denom > 1e-9 else "n/a"
        lines.append(
            "| {name} | {ol} | {asf} | {rnd} | {heur} | {ad} | {oh} | {gain} |".format(
                name=name,
