...
* Left: V2 latent graph world model vs Stage-2 MPNN baseline on
  fault-window MAE / zero-baseline ratio. Lower bar = better; the dashed
  line at 1.0 is the zero-delta baseline.

* Right: V4.2 imagined-planning agent hit rates with 95% bootstrap CI
  error bars across three evaluation splits — val (in-distribution),
  test (held-out trajectories, partial OOD because the world model has
  seen all fault types), and FT-7 (strict candidate-fault-type OOD with
  the agent trained on FT-{1,3} only). Reads point estimates +
  confidence ranges directly from the bootstrap_ci JSON files so the
  README never advertises a single-seed lucky number again.
...
CANONICAL_VAL = REPO_ROOT / "outputs" / "agent_v4_2_canonical" / "bootstrap_ci_val.json"
CANONICAL_TEST = REPO_ROOT / "outputs" / "agent_v4_2_canonical" / "bootstrap_ci_test.json"
OOD_FT7 = REPO_ROOT / "outputs" / "agent_v4_2_ood" / "bootstrap_ci_strict_ood.json"
...
def right_panel(ax: plt.Axes) -> None:
    ...
    ax.set_ylim(0, 1.05)
    ax.set_title(
        "V4.2 agent: discover hit > random on all splits, including strict OOD",
        fontsize=11, pad=8,
    )
    ax.legend(frameon=False, loc="upper right", fontsize=9)
    ax.grid(axis="y", alpha=0.25, linestyle=":")
...
def main() -> None:
    ...
    right_panel(axes[1])
    fig.suptitle(
        "wm-agent — latent graph world model + Dreamer-style imagined-planning agent",
