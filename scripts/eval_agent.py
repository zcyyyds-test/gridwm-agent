"""Evaluate a trained gridwm-agent world-model-distilled agent.
...
def main() -> int:
    ...
    ckpt = torch.load(args.agent_dir / "agent.pt", map_location=device, weights_only=True)
    cfg = dict(ckpt["agent_cfg"])
    if "discover_prior" not in ckpt["model"] and cfg.get("n_candidates") is not None:
        cfg["n_candidates"] = None
    agent = WorldModelDistilledRanker(**cfg).to(device)
