"""Train a world-model-distilled actor/critic agent on frozen gridwm-agent imagination."""
...
def main() -> int:
    ...
    parser.add_argument("--world-run-dir", type=Path, default=Path("outputs/demo_run"))
    ...
    parser.add_argument("--critic-ranking-margin-loss-weight", type=float, default=0.5)
    parser.add_argument(
    ...
    cfg = AgentTrainConfig(
        ...
        critic_ranking_margin_loss_weight=args.critic_ranking_margin_loss_weight,
        seed=args.seed,
    )
    ...
    def _build_agent() -> WorldModelDistilledRanker:
        return WorldModelDistilledRanker(
    ...
    def _init_pearson(model: WorldModelDistilledRanker) -> float:
        model.eval()
        with torch.no_grad():
