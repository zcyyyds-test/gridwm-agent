"""Evaluation metrics for wm-agent's Dreamer-style agent."""
...
def _ft7_heuristic_indices(metadata: list[dict[str, Any]], *, safe: bool) -> torch.Tensor:
    scores = []
    for row in metadata:
        fault_bonus = 10.0 if int(row["fault_type"]) == 7 else float(row["fault_type"])
        duration = float(row["duration_ms"]) / 180.0
        resistance = float(row["resistance_ohm"])
        resistance_score = 1.0 / max(resistance, 1e-6)
        scores.append(fault_bonus + duration + 0.05 * resistance_score)
...
def evaluate_agent_policy(
    ...
    *,
    ...
    ft7_idx = _ft7_heuristic_indices(metadata, safe=False).to(device).repeat(risk_raw.shape[0])
    ft7_risk = _gather_row(risk_raw, ft7_idx)
    ...
    return AgentEvalResult(
        ...
        ft7_heuristic_mean_risk=float(ft7_risk.mean().item()),
