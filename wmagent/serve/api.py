"""Localhost FastAPI surface for the gridwm-agent world-model system.
...
_DEFAULT_RUN_DIR = Path(os.environ.get("GRIDWM_RUN_DIR", "outputs/demo_run"))
_MAX_ANCHOR_INDEX = 4096
...
class SearchRequest(BaseModel):
    ...
    anchor_index: int = Field(default=0, ge=0, le=_MAX_ANCHOR_INDEX)
...
app = FastAPI(
    title="gridwm-agent World Model API",
...
def search(
    ...
    try:
        ...
        # Bad input: return a generic 400. Detailed error stays in server logs.
        _log.warning("search failed (%s): %s", type(exc).__name__, exc)
        raise HTTPException(status_code=400, detail="invalid request") from exc
    except Exception as exc:
        # Anything else is a server bug. Don't leak internals.
        _log.exception("internal error in /search")
