...
class MPNNDynamics(WorldModel):
    ...
    def predict_delta(
        ...
        *,
        ...
        h = self.node_in(node_in)

        a = self.action_mlp(action_global).unsqueeze(1).expand(-1, N, -1)
        h = h + a

        edge_attr = graph.edge_attr.to(state.device)  # (E, F_e), shared across batch
        edge_h = self.edge_in(edge_attr)              # (E, H)

        edge_index = graph.edge_index.to(state.device)
        outs = []
        for b in range(B):
            x = h[b]
            for blk in self.blocks:
                x = blk(x, edge_index, edge_h)
            outs.append(x)
